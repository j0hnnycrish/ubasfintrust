...
exports.up = function(knex) {
  return Promise.all([
    ...
    knex.schema.createTable('security_events', function(table) {
      ...
      table.string('event_type').notNullable(); // login_attempt, suspicious_activity, bot_detected, etc.
    ...
    knex.schema.createTable('ip_security', function(table) {
      table.uuid('id').primary().defaultTo(knex.raw('gen_random_uuid()'));
      table.string('ip_address').notNullable().unique();
      table.enum('status', ['whitelist', 'blacklist', 'greylist']).notNullable();
      table.string('reason');
      table.integer('threat_score').defaultTo(0); // 0-100 threat score
    ...
    // Rate limiting table
    knex.schema.createTable('rate_limits', function(table) {
      table.uuid('id').primary().defaultTo(knex.raw('gen_random_uuid()'));
      table.string('identifier').notNullable(); // IP address, user ID, etc.
      table.string('action').notNullable(); // login, transfer, api_call, etc.
      table.integer('attempts').defaultTo(1);
      ...
      table.index(['blocked', 'window_end']);
    }),

    // Device fingerprinting table
    knex.schema.createTable('device_fingerprints', function(table) {
      table.uuid('id').primary().defaultTo(knex.raw('gen_random_uuid()'));
      table.uuid('user_id').nullable().references('id').inTable('users').onDelete('CASCADE');
      table.string('fingerprint_hash').notNullable();
      table.jsonb('device_data'); // Screen resolution, timezone, plugins, etc.
      table.string('user_agent');
...
exports.down = function(knex) {
  return Promise.all([
    knex.schema.dropTableIfExists('device_fingerprints'),
    knex.schema.dropTableIfExists('rate_limits'),
    knex.schema.dropTableIfExists('ip_security'),
